 ...
 * @brief Source file for the PIDController class for configuring and
 ...
 */
PIDController::PIDController(SparkBase& base) : sparkBase(base) {}
    ...
    switch (static_cast<uint8_t>(controlType)) {
        ...
        default:
            throw std::invalid_argument(
                "Invalid control type. Must be CtrlType::kDutyCycle (Duty "
                "Cycle), CtrlType::kVelocity (Velocity), CtrlType::kVoltage "
                "(Voltage), CtrlType::kPosition (Position).");
    }
}
...
void PIDController::SetP(uint8_t slot, float p) { sparkBase.SetP(slot, p); }

void PIDController::SetI(uint8_t slot, float i) { sparkBase.SetI(slot, i); }

void PIDController::SetD(uint8_t slot, float d) { sparkBase.SetD(slot, d); }

void PIDController::SetF(uint8_t slot, float f) { sparkBase.SetF(slot, f); }
...
float PIDController::GetP(uint8_t slot) { return sparkBase.GetP(slot); }

float PIDController::GetI(uint8_t slot) { return sparkBase.GetI(slot); }

float PIDController::GetD(uint8_t slot) { return sparkBase.GetD(slot); }

float PIDController::GetF(uint8_t slot) { return sparkBase.GetF(slot); }

float PIDController::GetIZone(uint8_t slot) { return sparkBase.GetIZone(slot); }
