/**
 * @file SparkBase.cpp
 * @brief Source file for the base class for controlling REV Robotics SPARK
 * motor controllers
...

{
    ...
    if (bind(soc, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        ...
        throw std::runtime_error(RED
                                 "Binding to interface failed: Another program "
                                 "may be using this interface." RESET);
    }
}
...
void SparkBase::SendCanFrame(uint32_t deviceId, uint8_t dlc,
    ...
    for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
        ...
        if (bytesSent < 0) {
            ...
            throw std::runtime_error(RED "Failed to send CAN frame: " +
                                     std::string(strerror(errno)) + RESET);
        }
    }
    throw std::runtime_error(
...
void SparkBase::SendControlMessage(
    std::variant<MotorControl, SystemControl> command, std::string commandName,
    float value, std::optional<float> minValue,
    std::optional<float> maxValue) const {
    // Validate the value
    if (!std::isfinite(value)) {
        throw std::invalid_argument(RED + commandName +
                                    " must be a finite number." + RESET);
    }
    if (minValue && maxValue && (value < *minValue || value > *maxValue)) {
        throw std::out_of_range(RED + commandName + " must be between " +
...
uint64_t SparkBase::ReadPeriodicStatus(Status period) const {
    ...
    if (ret > 0) {
        ...
        if (bytesRead > 0) {
            ...
            uint32_t expectedArbitrationId =
                static_cast<uint32_t>(period) | deviceId;

            // If the arbitration ID matches, update the cached value
            if (receivedArbitrationId == expectedArbitrationId) {
                uint64_t newValue = 0;
                for (int i = 0; i < 8; i++) {
                    newValue |= static_cast<uint64_t>(response.data[i])
                                << (8 * i);
                }
...
void SparkBase::SetParameter(
    ...
    uint32_t arbitrationId =
        0x205C000 | (static_cast<uint32_t>(parameterId) << 6) | deviceId;
    std::array<uint8_t, 8> data = {};
    ...
    auto throwRangeError = [&](auto min, auto max) {
        if (customErrorMessage)
            ...
            throw std::out_of_range(RED + parameterName + " must be between " +
        ...
        [&](auto&& v) {
            ...
            if constexpr (std::is_same_v<T, float>) {
                if (!std::isfinite(v))  // Ensure float is valid
                    throw std::invalid_argument(RED + parameterName +
                                                " must be a finite number." +
                                                RESET);
                if (minValue && maxValue &&
                    (v < minValue.value() || v > maxValue.value()))
                    throwRangeError(minValue.value(), maxValue.value());
                std::memcpy(data.data(), &v,
                            sizeof(v));  // Copy float to CAN data
            } else if constexpr (std::is_integral_v<T>) {
                std::memcpy(data.data(), &v,
                            sizeof(v));  // Copy integer to CAN data
            } else if constexpr (std::is_same_v<T, bool>) {
                data[0] = v ? 1 : 0;  // Handle boolean type
            } else {
                throw std::invalid_argument(RED
                                            "Unsupported value type." RESET);
            }
        },
        value);
...
std::variant<float, uint32_t, bool> SparkBase::ReadParameter(
    ...
    uint32_t requestArbitrationId =
        0x205C000 | (static_cast<uint32_t>(parameterId) << 6) | deviceId;
    request.can_id =
        requestArbitrationId | CAN_EFF_FLAG;  // Use extended frame format
            ...
            if (receivedArbitrationId == requestArbitrationId) {
                ...
                switch (dataType) {
                    case 0x01: {
                        ...
                        for (int i = 0; i < 4; ++i) {
                            uintValue |= static_cast<uint32_t>(response.data[i])
                                         << (8 * i);
                        }
                        newValue = uintValue;
                        break;
...
void SparkBase::BurnFlash() {
    uint32_t arbitrationId =
        static_cast<uint32_t>(SystemControl::BurnFlash) + deviceId;
...
void SparkBase::FactoryReset() {
    uint32_t arbitrationId =
        static_cast<uint32_t>(SystemControl::FactoryReset) + deviceId;
...
void SparkBase::Identify() {
    uint32_t arbitrationId =
        static_cast<uint32_t>(SystemControl::Identify) + deviceId;
...
void SparkBase::ClearStickyFaults() {
    SendControlMessage(SystemControl::ClearStickyFaults, "ClearStickyFaults",
                       0.0f);
}
...
void SparkBase::SetDutyCycle(float dutyCycle) {
    SendControlMessage(MotorControl::DutyCycle, "Duty Cycle", dutyCycle, -1.0f,
                       1.0f);
}
...
void SparkBase::SetSmartVelocity(float smartVelocity) {
    SendControlMessage(MotorControl::SmartVelocity, "Smart Velocity",
                       smartVelocity);
}
...
void SparkBase::SetMotorType(MotorType type) {
    SetParameter(Parameter::kMotorType, PARAM_TYPE_UINT, "Motor Type",
...
void SparkBase::SetSensorType(SensorType type) {
    SetParameter(Parameter::kSensorType, PARAM_TYPE_UINT, "Sensor Type",
                 static_cast<uint8_t>(type), 0, 2,
                 "Invalid sensor type. Must be 0 (No Sensor), 1 (Hall Sensor), "
                 "or 2 (Encoder).");
}

void SparkBase::SetIdleMode(IdleMode mode) {
    SetParameter(Parameter::kIdleMode, PARAM_TYPE_UINT, "Idle Mode",
                 static_cast<uint8_t>(mode), 0, 1,
                 "Invalid idle mode. Must be 0 (Coast) or 1 (Brake).");
}

void SparkBase::SetInputDeadband(float deadband) {
    SetParameter(Parameter::kInputDeadband, PARAM_TYPE_FLOAT, "Input Deadband",
                 deadband);
}
...
void SparkBase::SetCtrlType(CtrlType type) {
    SetParameter(Parameter::kCtrlType, PARAM_TYPE_UINT, "Control Type",
                 static_cast<uint8_t>(type), 0, 3,
                 "Invalid control type. Must be 0 (Duty Cycle), 1 (Velocity), "
                 "2 (Voltage), or 3 (Position).");
}

void SparkBase::SetFeedbackSensorPID0(uint16_t sensor) {
    SetParameter(Parameter::kFeedbackSensorPID0, PARAM_TYPE_UINT,
                 "Feedback Sensor PID0", sensor);
}

void SparkBase::SetClosedLoopVoltageMode(uint8_t mode) {
    SetParameter(
        Parameter::kClosedLoopVoltageMode, PARAM_TYPE_UINT,
        "Closed Loop Voltage Mode", mode, 0, 2,
        "Invalid closed loop voltage mode. Must be 0 (Disabled), 1 (Control "
        "Loop Voltage Output Mode) or 2 (Voltage Compensation Mode).");
}

void SparkBase::SetCompensatedNominalVoltage(float voltage) {
    SetParameter(Parameter::kCompensatedNominalVoltage, PARAM_TYPE_FLOAT,
                 "Compensated Nominal Voltage", voltage);
}

void SparkBase::SetPositionPIDWrapEnable(bool enable) {
    SetParameter(Parameter::kPositionPIDWrapEnable, PARAM_TYPE_BOOL,
                 "Position PID Wrap Enable", enable);
}

void SparkBase::SetPositionPIDMinInput(float minInput) {
    SetParameter(Parameter::kPositionPIDMinInput, PARAM_TYPE_FLOAT,
                 "Position PID Min Input", minInput);
}

void SparkBase::SetPositionPIDMaxInput(float maxInput) {
    SetParameter(Parameter::kPositionPIDMaxInput, PARAM_TYPE_FLOAT,
                 "Position PID Max Input", maxInput);
}
...
void SparkBase::SetCurrentChop(float chop) {
    SetParameter(Parameter::kCurrentChop, PARAM_TYPE_FLOAT, "Current Chop",
                 chop, 0, 125);
}

void SparkBase::SetCurrentChopCycles(uint16_t cycles) {
    SetParameter(Parameter::kCurrentChopCycles, PARAM_TYPE_UINT,
                 "Current Chop Cycles", cycles);
}

void SparkBase::SetSmartCurrentStallLimit(uint16_t limit) {
    SetParameter(Parameter::kSmartCurrentStallLimit, PARAM_TYPE_UINT,
                 "Smart Current Stall Limit", limit);
}

void SparkBase::SetSmartCurrentFreeLimit(uint16_t limit) {
    SetParameter(Parameter::kSmartCurrentFreeLimit, PARAM_TYPE_UINT,
                 "Smart Current Free Limit", limit);
}

void SparkBase::SetSmartCurrentConfig(uint16_t config) {
    SetParameter(Parameter::kSmartCurrentConfig, PARAM_TYPE_UINT,
                 "Smart Current Config", config);
}
...
void SparkBase::SetP(uint8_t slot, float p) {
    static const std::array<Parameter, 4> params = {
        Parameter::kP_0, Parameter::kP_1, Parameter::kP_2, Parameter::kP_3};

    if (slot >= params.size()) {
        throw std::out_of_range(RED
                                "Invalid slot number. Max value is 3." RESET);
    }
...
void SparkBase::SetI(uint8_t slot, float i) {
    static const std::array<Parameter, 4> params = {
        Parameter::kI_0, Parameter::kI_1, Parameter::kI_2, Parameter::kI_3};
...
void SparkBase::SetD(uint8_t slot, float d) {
    static const std::array<Parameter, 4> params = {
        Parameter::kD_0, Parameter::kD_1, Parameter::kD_2, Parameter::kD_3};
...
void SparkBase::SetF(uint8_t slot, float f) {
    static const std::array<Parameter, 4> params = {
        Parameter::kF_0, Parameter::kF_1, Parameter::kF_2, Parameter::kF_3};
...
void SparkBase::SetIZone(uint8_t slot, float iZone) {
    static const std::array<Parameter, 4> params = {
        Parameter::kIZone_0, Parameter::kIZone_1, Parameter::kIZone_2,
        Parameter::kIZone_3};
...
void SparkBase::SetDFilter(uint8_t slot, float dFilter) {
    static const std::array<Parameter, 4> params = {
        Parameter::kDFilter_0, Parameter::kDFilter_1, Parameter::kDFilter_2,
...
void SparkBase::SetHardLimitFwdEn(bool enable) {
    SetParameter(Parameter::kHardLimitFwdEn, PARAM_TYPE_BOOL,
                 "Hard Limit Forward Enable", enable);
}

void SparkBase::SetHardLimitRevEn(bool enable) {
    SetParameter(Parameter::kHardLimitRevEn, PARAM_TYPE_BOOL,
                 "Hard Limit Reverse Enable", enable);
}

void SparkBase::SetLimitSwitchFwdPolarity(bool polarity) {
    SetParameter(Parameter::kLimitSwitchFwdPolarity, PARAM_TYPE_BOOL,
                 "Limit Switch Forward Polarity", polarity);
}

void SparkBase::SetLimitSwitchRevPolarity(bool polarity) {
    SetParameter(Parameter::kLimitSwitchRevPolarity, PARAM_TYPE_BOOL,
                 "Limit Switch Reverse Polarity", polarity);
}

void SparkBase::SetSoftLimitFwdEn(bool enable) {
    SetParameter(Parameter::kSoftLimitFwdEn, PARAM_TYPE_BOOL,
                 "Soft Limit Forward Enable", enable);
}

void SparkBase::SetSoftLimitRevEn(bool enable) {
    SetParameter(Parameter::kSoftLimitRevEn, PARAM_TYPE_BOOL,
                 "Soft Limit Reverse Enable", enable);
}

void SparkBase::SetSoftLimitFwd(float limit) {
    SetParameter(Parameter::kSoftLimitFwd, PARAM_TYPE_FLOAT,
                 "Soft Limit Forward", limit);
}

void SparkBase::SetSoftLimitRev(float limit) {
    SetParameter(Parameter::kSoftLimitRev, PARAM_TYPE_FLOAT,
                 "Soft Limit Reverse", limit);
}
...
void SparkBase::SetFollowerConfig(uint32_t config) {
    SetParameter(Parameter::kFollowerConfig, PARAM_TYPE_UINT, "Follower Config",
                 config);
}
...
void SparkBase::SetEncoderCountsPerRev(uint16_t counts) {
    SetParameter(Parameter::kEncoderCountsPerRev, PARAM_TYPE_UINT,
                 "Encoder Counts Per Revolution", counts);
}

void SparkBase::SetEncoderAverageDepth(uint8_t depth) {
    SetParameter(Parameter::kEncoderAverageDepth, PARAM_TYPE_UINT,
                 "Encoder Average Depth", depth, 1, 64);
}

void SparkBase::SetEncoderSampleDelta(uint8_t delta) {
    SetParameter(Parameter::kEncoderSampleDelta, PARAM_TYPE_UINT,
                 "Encoder Sample Delta", delta, 1, 255);
}

void SparkBase::SetEncoderInverted(bool inverted) {
    SetParameter(Parameter::kEncoderInverted, PARAM_TYPE_BOOL,
                 "Encoder Inverted", inverted);
}

void SparkBase::SetPositionConversionFactor(float factor) {
    SetParameter(Parameter::kPositionConversionFactor, PARAM_TYPE_FLOAT,
                 "Position Conversion Factor", factor);
}

void SparkBase::SetVelocityConversionFactor(float factor) {
    SetParameter(Parameter::kVelocityConversionFactor, PARAM_TYPE_FLOAT,
                 "Velocity Conversion Factor", factor);
}

void SparkBase::SetClosedLoopRampRate(float rampRate) {
    SetParameter(Parameter::kClosedLoopRampRate, PARAM_TYPE_FLOAT,
                 "Closed Loop Ramp Rate", rampRate);
}

void SparkBase::SetHallSensorSampleRate(float rate) {
    SetParameter(Parameter::kHallSensorSampleRate, PARAM_TYPE_FLOAT,
                 "Hall Sensor Sample Rate", rate);
}

void SparkBase::SetHallSensorAverageDepth(uint16_t depth) {
    SetParameter(Parameter::kHallSensorAverageDepth, PARAM_TYPE_UINT,
                 "Hall Sensor Average Depth", depth);
}
...
void SparkBase::SetSmartMotionMaxVelocity(uint8_t slot, float maxVel) {
    ...
    SetParameter(params[slot], PARAM_TYPE_FLOAT, "Smart Motion Max Velocity",
                 maxVel);
}
...
void SparkBase::SetSmartMotionMaxAccel(uint8_t slot, float maxAccel) {
    ...
    SetParameter(params[slot], PARAM_TYPE_FLOAT, "Smart Motion Max Accel",
                 maxAccel);
}
...
void SparkBase::SetSmartMotionMinVelOutput(uint8_t slot, float minVel) {
    ...
    SetParameter(params[slot], PARAM_TYPE_FLOAT, "Smart Motion Min Vel Output",
                 minVel);
}
...
void SparkBase::SetSmartMotionAllowedClosedLoopError(uint8_t slot,
    ...
    SetParameter(params[slot], PARAM_TYPE_FLOAT,
                 "Smart Motion Allowed Close Loop Error", error);
}
...
void SparkBase::SetSmartMotionAccelStrategy(uint8_t slot, float strategy) {
    ...
    SetParameter(params[slot], PARAM_TYPE_FLOAT, "Smart Motion Accel Strategy",
                 strategy);
}
...
void SparkBase::SetAnalogPositionConversion(float factor) {
    SetParameter(Parameter::kAnalogPositionConversion, PARAM_TYPE_FLOAT,
                 "Analog Position Conversion", factor);
}

void SparkBase::SetAnalogVelocityConversion(float factor) {
    SetParameter(Parameter::kAnalogVelocityConversion, PARAM_TYPE_FLOAT,
                 "Analog Velocity Conversion", factor);
}

void SparkBase::SetAnalogAverageDepth(uint16_t depth) {
    SetParameter(Parameter::kAnalogAverageDepth, PARAM_TYPE_UINT,
                 "Analog Average Depth", depth);
}

void SparkBase::SetAnalogSensorMode(uint8_t mode) {
    SetParameter(
        Parameter::kAnalogSensorMode, PARAM_TYPE_UINT, "Analog Sensor Mode",
        mode, 0, 1,
        "Invalid analog sensor mode. Must be 0 (Absolute) or 1 (Relative).");
}

void SparkBase::SetAnalogInverted(bool inverted) {
    SetParameter(Parameter::kAnalogInverted, PARAM_TYPE_BOOL, "Analog Inverted",
                 inverted);
}

void SparkBase::SetAnalogSampleDelta(uint16_t delta) {
    SetParameter(Parameter::kAnalogSampleDelta, PARAM_TYPE_UINT,
                 "Analog Sample Delta", delta);
}

// Alternate Encoder //

void SparkBase::SetDataPortConfig(uint8_t config) {
    SetParameter(Parameter::kDataPortConfig, PARAM_TYPE_UINT,
                 "Data Port Config", config, 0, 1,
                 "Invalid data port config. Must be 0 (Default) or 1 "
                 "(Alternate Encoder Mode).");
}

void SparkBase::SetAltEncoderCountsPerRev(uint16_t counts) {
    SetParameter(Parameter::kAltEncoderCountsPerRev, PARAM_TYPE_UINT,
                 "Alternate Encoder Counts Per Revolution", counts);
}

void SparkBase::SetAltEncoderAverageDepth(uint8_t depth) {
    SetParameter(Parameter::kAltEncoderAverageDepth, PARAM_TYPE_UINT,
                 "Alternate Encoder Average Depth", depth, 1, 64);
}

void SparkBase::SetAltEncoderSampleDelta(uint8_t delta) {
    SetParameter(Parameter::kAltEncoderSampleDelta, PARAM_TYPE_UINT,
                 "Alternate Encoder Sample Delta", delta, 1, 255);
}

void SparkBase::SetAltEncoderInverted(bool inverted) {
    SetParameter(Parameter::kAltEncoderInverted, PARAM_TYPE_BOOL,
                 "Alternate Encoder Inverted", inverted);
}

void SparkBase::SetAltEncoderPositionFactor(float factor) {
    SetParameter(Parameter::kAltEncoderPositionFactor, PARAM_TYPE_FLOAT,
                 "Alternate Encoder Position Factor", factor);
}

void SparkBase::SetAltEncoderVelocityFactor(float factor) {
    SetParameter(Parameter::kAltEncoderVelocityFactor, PARAM_TYPE_FLOAT,
...
void SparkBase::SetDutyCyclePositionFactor(float factor) {
    SetParameter(Parameter::kDutyCyclePositionFactor, PARAM_TYPE_FLOAT,
                 "Duty Cycle Position Factor", factor);
}

void SparkBase::SetDutyCycleVelocityFactor(float factor) {
    SetParameter(Parameter::kDutyCycleVelocityFactor, PARAM_TYPE_FLOAT,
                 "Duty Cycle Velocity Factor", factor);
}

void SparkBase::SetDutyCycleInverted(bool inverted) {
    SetParameter(Parameter::kDutyCycleInverted, PARAM_TYPE_BOOL,
                 "Duty Cycle Inverted", inverted);
}

void SparkBase::SetDutyCycleAverageDepth(uint8_t depth) {
    SetParameter(
        Parameter::kDutyCycleAverageDepth, PARAM_TYPE_UINT,
        "Duty Cycle Average Depth", depth, 0, 7,
        "Invalid average depth. Must be 0 (1 bit), 2 (2 bits), 3 (4 bits), 4 "
        "(8 bits), 5 (16 bits), 6 (32 bits), or 7 (64 bits).");
}

void SparkBase::SetDutyCyclePrescalar(uint8_t prescalar) {
    SetParameter(Parameter::kDutyCyclePrescalar, PARAM_TYPE_UINT,
                 "Duty Cycle Prescalar", prescalar, 0, 71);
}

void SparkBase::SetDutyCycleZeroOffset(float offset) {
    SetParameter(Parameter::kDutyCycleZeroOffset, PARAM_TYPE_FLOAT,
                 "Duty Cycle Zero Offset", offset, 0.0f, 1.0f);
}
...
float SparkBase::GetCompensatedNominalVoltage() {
    ...
    throw std::runtime_error(
        "Invalid type returned for CompensatedNominalVoltage");
}
...
uint16_t SparkBase::GetSmartCurrentStallLimit() {
    ...
    throw std::runtime_error(
        "Invalid type returned for SmartCurrentStallLimit");
}
...
bool SparkBase::GetLimitSwitchFwdPolarity() {
    ...
    throw std::runtime_error(
        "Invalid type returned for LimitSwitchFwdPolarity");
}
...
bool SparkBase::GetLimitSwitchRevPolarity() {
    ...
    throw std::runtime_error(
        "Invalid type returned for LimitSwitchRevPolarity");
}
...
float SparkBase::GetPositionConversionFactor() {
    ...
    throw std::runtime_error(
        "Invalid type returned for PositionConversionFactor");
}
...
float SparkBase::GetVelocityConversionFactor() {
    ...
    throw std::runtime_error(
        "Invalid type returned for VelocityConversionFactor");
}
...
uint16_t SparkBase::GetHallSensorAverageDepth() {
    ...
    throw std::runtime_error(
        "Invalid type returned for HallSensorAverageDepth");
}
...
float SparkBase::GetSmartMotionMaxVelocity(uint8_t slot) {
    ...
    throw std::runtime_error(
        "Invalid type returned for SmartMotionMaxVelocity");
}
...
float SparkBase::GetSmartMotionMinVelOutput(uint8_t slot) {
    ...
    throw std::runtime_error(
        "Invalid type returned for SmartMotionMinVelOutput");
}
...
float SparkBase::GetSmartMotionAllowedClosedLoopError(uint8_t slot) {
    ...
    throw std::runtime_error(
        "Invalid type returned for SmartMotionAllowedClosedLoopError");
}
...
float SparkBase::GetSmartMotionAccelStrategy(uint8_t slot) {
    ...
    throw std::runtime_error(
        "Invalid type returned for SmartMotionAccelStrategy");
}
...
float SparkBase::GetAnalogPositionConversion() {
    ...
    throw std::runtime_error(
        "Invalid type returned for AnalogPositionConversion");
}
...
float SparkBase::GetAnalogVelocityConversion() {
    ...
    throw std::runtime_error(
        "Invalid type returned for AnalogVelocityConversion");
}
...
uint16_t SparkBase::GetAltEncoderCountsPerRev() {
    ...
    throw std::runtime_error(
        "Invalid type returned for AltEncoderCountsPerRev");
}
...
uint8_t SparkBase::GetAltEncoderAverageDepth() {
    ...
    throw std::runtime_error(
        "Invalid type returned for AltEncoderAverageDepth");
}
...
float SparkBase::GetAltEncoderPositionFactor() {
    ...
    throw std::runtime_error(
        "Invalid type returned for AltEncoderPositionFactor");
}
...
float SparkBase::GetAltEncoderVelocityFactor() {
    ...
    throw std::runtime_error(
        "Invalid type returned for AltEncoderVelocityFactor");
}
...
float SparkBase::GetDutyCyclePositionFactor() {
    ...
    throw std::runtime_error(
        "Invalid type returned for DutyCyclePositionFactor");
}
...
float SparkBase::GetDutyCycleVelocityFactor() {
    ...
    throw std::runtime_error(
        "Invalid type returned for DutyCycleVelocityFactor");
}
