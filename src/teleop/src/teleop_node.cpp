...
class Teleop : public rclcpp::Node {
   public:
    Teleop() : Node("Teleop") {
        ...
        drivetrainPub = this->create_publisher<sensor_msgs::msg::JointState>(
            "drivetrain_cmd_vel", 10);

        armPub =
            this->create_publisher<std_msgs::msg::String>("shovel/arm_cmd", 10);

        scoopPub = this->create_publisher<std_msgs::msg::String>(
            "shovel/scoop_cmd", 10);

        bucketPub = this->create_publisher<std_msgs::msg::String>(
            "shovel/bucket_cmd", 10);

        service = this->create_service<interfaces::srv::SetTeleop>(
            "set_teleop",
            [this](const std::shared_ptr<interfaces::srv::SetTeleop::Request>
                       request,
                   std::shared_ptr<interfaces::srv::SetTeleop::Response>
                       response) { this->set_teleop(request, response); });
    }
   ...
   private:
    ...
    void set_teleop(
        const std::shared_ptr<interfaces::srv::SetTeleop::Request> request,
